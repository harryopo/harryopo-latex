...
设 $x$ 和 $y$ 是两个变量，$D$ 是一个给定的数集。如果对于每个数 $x \in D$，变量 $y$ 按照一定的法则总有确定的数值和它对应，则称 $y$ 是 $x$ 的**函数**，记作 $y = f(x)$。

这里包含两个核心要素：

- **定义域**：自变量 $x$ 的取值范围 $D$
- **对应法则**：$f$ 表示 $x$ 与 $y$ 之间的对应关系

### 函数的表示法

函数主要有三种表示法：

1. **解析法**（公式法）：$y = x^2 + 2x + 1$
2. **表格法**：用表格列出 $x$ 与 $y$ 的对应值
3. **图像法**：用坐标系中的曲线表示函数关系
...
> **$\varepsilon$-$\delta$ 定义**：设函数 $f(x)$ 在点 $x_0$ 的某个去心邻域内有定义。若 $\forall \varepsilon > 0$，$\exists \delta > 0$，使得当 $0 < |x - x_0| < \delta$ 时，恒有 $|f(x) - A| < \varepsilon$，则称 $A$ 为 $f(x)$ 当 $x \to x_0$ 时的极限，记作：
