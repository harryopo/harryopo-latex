...
\begin{kaobox}[frametitle=待办]
需要完成的一个小任务是更新\Command{sidenote}命令，使其只接受一个可选参数，即偏移量。特殊标记参数可以放到其他地方。换言之，我们希望\Command{sidenote}的语法与\Command{marginnote}相似。
\end{kaobox}
...
在某些情况下，当你有一小段代码，而这段代码直接放在正文中会打断叙述流程，这时候你可以使用\Environment{marginlisting}环境。对于这一特性的支持仍然有限，尤其是对于标题，但你可以尝试以下代码。
...
边栏展示元素时也会遇到的布局问题，那就是：用户需要手动调整代码列表和其标题之间的间距。
