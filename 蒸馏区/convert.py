...
def assemble_paper(blocks: List[Tuple[str, str]], title: str, author: str,
    ...
    bib_lines: List[str] = []
    first_h1 = True
    meta_re = re.compile(r'^(作者|摘要|关键词)[：:]')
    keywords_in_text_re = re.compile(r'(?m)^关键词[：:].+$')
    ...
    for btype, content in blocks:
        ...
        if btype == BLOCK_H1:
            ...
            # ## → \section*（映射到 paper.cls 的 section 层级：大标宋+蓝色）
            lines.append(r"\section*{" + _clean_title(content) + "}")
            lines.append(r"\addcontentsline{toc}{section}{" + _clean_title(content) + "}")
            lines.append("")
        elif btype == BLOCK_H3:
            # ### → \subsection*（映射到 paper.cls 的 subsection 层级：黑体+辅色）
            ...
            lines.extend(_parse_table_to_latex(content))
...
def _parse_table_to_latex(raw: str) -> List[str]:
    """将 Markdown 表格转为 booktabs 三线表（tabularx 自适应宽度 + caption 下方）"""
...
def convert_md_to_tex(
    ...
    if not abstract:
        m = re.search(r'(?ims)^摘要[：:]?\s*\n?(.+?)(?=\n(?:关键词[：:]|#|##))', text)
        if m:
            abstract = m.group(1).strip()
    if not keywords:
        m = re.search(r'(?im)^关键词[：:]\s*(.+)$', text, re.MULTILINE)
        if m:
