...
| 层 | 模块 | 职责 |
|----|------|------|
| 入口编排 | `office.py` | 11 子命令：render（word/paper/notes/slides 四链路）/ template / diagram / redline / revise / live / trace / style / govcheck / ide / info；路径常量单一事实来源（CLS_DIR/FONTS_DIR/PAPER_DIR/NOTES_DIR/SLIDES_DIR）；ASCII 护栏；环境探测（tex/pandoc 补 PATH） |
...
| **IDE 配置分发** | `office.py ide [--dest 项目根]` | 生成 .vscode/settings.json（LaTeX Workshop xelatex 配方 + 方正 MD 预览 CSS），幂等合并不抹用户配置（2026-09-18 新增） |

### 4.2 修订审阅（改稿双向留痕）
- **redline**：`office.py redline 初稿.docx 修改稿.docx -o 红线稿.docx [--engine wmlcomparer|docxdiff]`——原生 w:ins/w:del，免装 Word
- **style（样式保真，style_profile.py）**：`office.py style extract 模板.docx -o x.json` / `style check 产物.docx --profile x.json`——sectPr 边距+Normal/Heading1-3 字体字号行距档案（style-profile-v1），行距结构化 multiple/exact/at_least；容差 字号±0.5pt 边距±0.05cm
...
**TODO 清单（方案书 v3 §6 未动项）**：✅ P2 演示文稿链路已完成（2026-09-18 定调 beamer/PDF 路线，harryopo-slides 三主题 + MD 自动链路，不做可编辑 .pptx）；✅ 公文 Word 模板国标化已完成（2026-09-18 `--format word --gov` + govcheck 8/8 闭环）；✅ IDE 配置分发已完成（2026-09-18 `office.py ide`，LaTeX Workshop 配方 + 预览 CSS 幂等分发）；✅ word-mcp-live 适配层已完成（2026-09-20 `office.py live`：COM 实时修订会话 status/edit/comment/accept/reject，CLI 契约即适配器；实测三坑记录在 SKILL.md）；✅ Citra 证据回溯已完成（2026-09-20 `office.py trace` 自研——调研核实 Citra 系方案书信息幻觉，GitHub 无此工具；实测 31/31 段落全中+编造句正确 MISS）；✅ 模板注册表 v2 样式保真校验已完成（2026-09-20 `office.py style` extract/check，style-profile-v1 schema）。**P2 至此 5/5 全部完成**；⬜ P3 — Typst 通道、模板市场/多人协作、pdfcpu 后处理、HermesOffice 往返对标。代码内 TODO：【信息缺失——脚本注释无显式 TODO 标记，以上以方案书为准】
