...
def cmd_render(args):
    ...
    for _lang, _block in _re.findall(r'```([^\n`]*)\n(.*?)```', md_text, _re.DOTALL):
        if _lang.strip().lower() in ('mermaid', 'super-diagram'):
            continue
        _n_box = sum(1 for c in _block if c in _BOX_CHARS)
        _n_ascii_box = len(_re.findall(r'\+[-=]{3,}\+', _block))
